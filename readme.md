...
This repo provides a minimal implementation of the gradeddag consensus protocol. The codebase has been designed to be small, efficient, and easy to benchmark and modify. It has not been designed to run in production but uses real cryptography (kyber), networking(native), and storage (nutsdb).

Say something about gradeddag...

## Quick Start

gradeddag is written in Golang, but all benchmarking scripts are written in Python and run with Fabric. To deploy and benchmark a testbed of 4 nodes on your local machine, clone the repo and install the python dependencies:

```shell
git clone https://github.com/ac-dcz/mempool-GradedDag
cd mempool-GradedDag/benchmark
...
The file [settings.json](https://github.com/ac-dcz/mempool-GradedDag/tree/main/benchmark/settings.json) located in [gradeddag/benchmark](https://github.com/ac-dcz/mempool-GradedDag/tree/main/benchmark) contains all the configuration parameters of the testbed to deploy. Its content looks as follows:
...
The Aliyun instances are orchestrated with [Fabric](http://www.fabfile.org/) from the file [fabfile.py](https://github.com/ac-dcz/gradeddag/blob/main/benchmark/fabfile.py) (located in [gradeddag/benchmark](https://github.com/ac-dcz/gradeddag/tree/main/benchmark)) you can list all possible commands as follows:

The command `fab create` creates new Aliyun instances; open [fabfile.py](https://github.com/ac-dcz/gradeddag/blob/main/benchmark/fabfile.py) and locate the `create` task:
...
The parameter `nodes` determines how many instances to create in _each_ Aliyun region. That is, if you specified 4 Aliyun regions as in the example of step 3, setting `nodes=2` will creates a total of 8 machines:
